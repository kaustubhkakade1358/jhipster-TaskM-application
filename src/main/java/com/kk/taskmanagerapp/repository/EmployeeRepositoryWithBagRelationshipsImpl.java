...
public class EmployeeRepositoryWithBagRelationshipsImpl implements EmployeeRepositoryWithBagRelationships {
    ...
    public Optional<Employee> fetchBagRelationships(Optional<Employee> employee) {
        return employee.map(this::fetchRoles);
    }
    ...
    public List<Employee> fetchBagRelationships(List<Employee> employees) {
        return Optional.of(employees).map(this::fetchRoles).orElse(Collections.emptyList());
    }

    Employee fetchRoles(Employee result) {
        return entityManager
            .createQuery(
                "select employee from Employee employee left join fetch employee.roles where employee is :employee",
                Employee.class
            )
    ...
    List<Employee> fetchRoles(List<Employee> employees) {
        ...
        List<Employee> result = entityManager
            .createQuery(
                "select distinct employee from Employee employee left join fetch employee.roles where employee in :employees",
                Employee.class
            )
